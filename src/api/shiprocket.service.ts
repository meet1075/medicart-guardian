import { FullOrder } from "@/hooks/use-orders";

// Minimal cache to avoid hitting auth endpoint for every order
...
export async function getShiprocketToken(): Promise<string> {
  ...
  tokenExpiry = Date.now() + 9 * 24 * 60 * 60 * 1000; // valid for 10 days, refreshing after 9
  return cachedToken!;
}
...
export async function createShiprocketOrder(order: any) {
  ...
  const payload = {
    ...
    pickup_location: "Primary", // Must match the configured pickup location in Shiprocket dashboard
...
export async function generateAWB(shipmentId: string) {
  ...
  if (token === "MOCK_TOKEN_123") {
    return {
      ...
      response: {
        data: {
          awb_code: `AWB${Math.floor(Math.random() * 100000000)}IN`,
          applied_weight: 0.5,
          company_id: 12345,
...
export async function cancelShipment(awbs: string[]) {
  ...
  if (token === "MOCK_TOKEN_123") {
    return { status: 200, message: "Mock shipment cancelled successfully" };
  }
